...
const create = async (user) => {
    ...
    if (mdl_cohort.length) {
        ...
        if (!mdl_user.length) {
            await sequelize.query(`INSERT INTO mdl_user 
        ...
        let [corte, cursos] = await Promise.all([
            ...
            sequelize.query(`SELECT mdl_enrol.id AS id_enrol, mdl_context.id AS id_context,  
        ...
        if (assignments.length && enrolments.length) {
            await Promise.all([
                mdl_role_assignments.bulkCreate(assignments),
                mdl_user_enrolments.bulkCreate(enrolments)
            ])
        }
    } else {
...
const enroll = async (user) => {
    ...
    if (!mdluser.length) {
        ...
        await sequelize.query(`UPDATE mdl_user SET username = :usuario, password = md5(:password)
...
const quiz = async (courseid, userid) => {
    const moodleExamenes = await sequelize.query(`SELECT mdl_quiz.id, mdl_quiz.name, 
