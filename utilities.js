...
const notificacionSMS = async (notificacion) => {
    ...
    try {
        const { data } = await axios.post(`https://api103.hablame.co/api/sms/v3/send/marketing`,
            {
...
const notificacionEmail = async (notificacion, plantilla, datos) => {
        ...
        if (email_plantillas) {
            ...
            if (plantilla.resp_plantilla) {
                ...
                plantilla.resp_plantilla.adm_email_plantillas_resps.forEach(resp => {
                    ...
                    const email = resp.id_usuario_usuario.correos || resp.id_usuario_usuario.correo || resp.id_usuario_usuario.email;
    ...
    if (notificacion?.html) {
        const transporter = nodemailer.createTransport({
            ...
            auth: {
                user: 'AKIAXLXDIFEY7H4VIM7M',
                pass: 'BIuXoqf2nV8I8+XeNPxJ3SdrN/N7j70cqdgSzs+PUGWQ'
            }
        });

        const mailOptions = {
            from: '"Cartagena International School" <no-replay@cartagenainternationalschool.com>', // sender address
